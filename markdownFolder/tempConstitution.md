...
- **C# 最佳實踐**: 使用C# 14 最新功能、檔案範圍命名空間、模式比對、null 安全(`is null`/`is not null`)
...
- **資料庫查詢優化**: 避免N+1 查詢問題、使用適當索引、實施查詢計劃分析
...
- **SQL 注入保護**: 使用Entity Framework Core 參數化查詢,避免字串拼接SQL
- **敏感資料保護**: 密碼、金鑰、連線字串等必須使用Secret Manager 或Azure Key Vault
...
- **語言**: C# 14 (啟用最新功能)
- **資料庫**: JSON 文件存儲 (如適用, 這是練習用不會公開網路使用.)
...
- **功能檔案夾**: 大型功能使用Feature Folders 或領域驅動設計原則組織
